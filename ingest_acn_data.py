...
- Structured event extraction for EventStore
"""
...
class AsyncACNScraper:
    ...
    def is_event_page(self, url: str) -> bool:
        ...
        # Only top-level event detail pages
        if '/events/summits/' in path and path.count('/') <= 4:
            return True
        if '/events/conference/' in path and path.count('/') <= 4:
            return True
...
# =============================================================================
# EVENT EXTRACTION BUILDER
# =============================================================================

class EventExtractionBuilder:
    """
    Extract structured event data from scraped pages.
    Creates events.json with structured event/summit data for EventStore.
    """
    ...
    def extract_events(self):
        """
        Process scraped pages and extract structured event data.
        Populates EventStore with events from acn_data/raw pages.
        """
        try:
            ...
            print("\n" + "="*80)
            print("PHASE 3: Structured Event Extraction")
            print("="*80 + "\n")
            ...
            print(f"  Events stored at: {self.config.DATA_DIR}/events.json")
            
            return store
            
        except ImportError as e:
            logger.warning(f"Event extraction modules not available: {e}")
            logger.info("Skipping event extraction phase")
            return None
        except Exception as e:
...
async def main():
    ...
    # Extract structured events for EventStore
    event_builder = EventExtractionBuilder(config)
    event_store = event_builder.extract_events()
    ...
    print("  ✓ Structured events populated in EventStore")
    print("\nEvent queries will now bypass vector RAG and use structured data.")
    print("\nTest your queries with:")
    print("  python query_acn_rag.py")
    print("  or")
    print("  python api.py")
